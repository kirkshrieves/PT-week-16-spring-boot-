...
public class GlobalErrorHandler {
	private enum LogStatus{
		STACK_TRACE, MESSAGE_ONLY//for logging error messages
	}
	
	@ExceptionHandler(MethodArgumentTypeMismatchException.class)
	@ResponseStatus(code = HttpStatus.BAD_REQUEST)
	public Map<String, Object> handleMethodArgumentTypeMismatchExceptionn(
	...
	private Map<String, Object> createExceptionMessage(Exception e, 
		...
		} //this fixes the junit test error for expecting no such element exception but
		// expected "uri"="/jeeps"
		
		error.put("message", e.toString());
		error.put("status code", status.value()); 
		//error.put("uri", webRequest.getContextPath());//deleted code; see if statement above
		...
		if(logStatus == LogStatus.MESSAGE_ONLY) {
			...
			//.equals() isn't needed for enums because they're ordinal values
...
/*
* This helped the testing return a 404 status error that was originally a 500 status 
* when the partial testThatAnErrorMessageIsReturnedWhenAnInvalidTrimIsSupplied was ran
* This ^ is before adding the error.put line
*/